...
A production-ready RESTful service built with Spring Boot, featuring enterprise-grade security, role-based validation, and comprehensive error handling.

**Original Repository:** This project is based on the Spring Guide tutorial repository (https://github.com/spring-guides/tut-rest) but has been heavily modified to showcase additional security features and modern REST API best practices.

== ✨ Enhanced Features

=== 🔐 Security Features
* **JWT Authentication** - Stateless authentication with Bearer tokens (30-minute expiry)
* **Protected Endpoints** - All API endpoints secured except login
* **Spring Security Integration** - Complete security configuration with JWT filter
* **Secure Credentials** - Hardcoded admin credentials for demo purposes

=== ✅ Validation & Data Integrity
* **Role-based Validation** - Custom enum validation for employee roles
* **Supported Roles:** Developer, Manager, Designer, QA Engineer, DevOps, Product Manager
* **Input Validation** - Comprehensive request validation with meaningful error messages
* **Data Consistency** - Role constraints ensure data integrity

=== 🛡️ Error Handling
* **JSON Error Responses** - Consistent error format across all endpoints
* **Custom Exception Handlers** - Separate handlers for different error types
* **HTTP Status Compliance** - Proper status codes (400, 401, 404, 500)
...
* **Error Scenarios** - Comprehensive error response examples
...
* Java 17 or later
* Maven 3.6+
* Your favorite IDE (IntelliJ IDEA, VSCode, etc.)

=== Running the Application

1. **Clone and navigate:**
```bash
git clone https://github.com/debasisj/tut-rest.git
cd tut-rest/rest
```
...
curl -X POST http://localhost:8080/auth/login \
  ...
  -d '{"userId": "admin", "password": "G5hnX2GMyFEetC1"}'
```
...
=== Authentication
* `POST /auth/login` - Login with credentials to get JWT token

=== Employee Management
* `GET /employees` - List all employees (with HATEOAS links)
...
│   ├── Employee.java              # Domain entity with role validation
│   ├── EmployeeController.java    # REST endpoints with HATEOAS
│   ├── EmployeeRepository.java    # JPA repository
│   ├── Role.java                  # Role validation enum
│   ├── PayrollApplication.java    # Main application
│   ├── LoadDatabase.java          # Test data preloader
│   ├── *Exception.java           # Custom exceptions
│   ├── *Advice.java              # Exception handlers
...
=== 2. Create Employee with Valid Role
```bash
...
curl -X POST http://localhost:8080/employees \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name": "Jane Doe", "role": "InvalidRole"}'
```
...
public class SecurityConfig {
    // Protects all endpoints except /auth/login
...
The complete OpenAPI 3.0 specification is available in `openapi.yaml` and includes:

* **Authentication endpoints** with JWT examples
* **Employee CRUD operations** with validation details  
* **Security schemes** for Bearer token authentication
* **Error responses** for all failure scenarios
* **HATEOAS examples** showing hypermedia links
* **Role validation** constraints and examples

== 🚦 Error Handling

The API provides comprehensive error handling with consistent JSON responses:

* **400 Bad Request** - Invalid input, role validation failures
* **401 Unauthorized** - Missing or invalid authentication
* **404 Not Found** - Resource not found
* **500 Internal Server Error** - Server-side errors

All errors include descriptive messages to aid in debugging and client implementation.

== 🎯 Key Modifications from Original

This repository demonstrates several enhancements over the basic Spring REST tutorial:

1. **Complete JWT Authentication System** - Stateless authentication with proper token management
2. **Role-based Validation** - Custom enum validation with meaningful error messages  
3. **Comprehensive Error Handling** - JSON error responses for all scenarios
4. **Security Integration** - Full Spring Security configuration with JWT filters
5. **Production-ready Features** - HATEOAS, OpenAPI documentation, proper HTTP status codes
6. **Testing Examples** - Complete curl examples for all endpoints and error scenarios

This makes it suitable for understanding modern REST API development practices and enterprise-grade security implementation.

== 📝 License

This project is based on the Spring Guide tutorial and is provided for educational purposes.
...
* The `role` field uses an enum with validation to ensure only valid roles are accepted.
...
To ensure data integrity, we implement role validation using a custom enum:

.rest/src/main/java/payroll/Role.java
[%collapsible%open]
====
...
This enum ensures that only predefined roles can be assigned to employees, providing both validation and consistency.
...
Modern REST APIs require robust security. We implement JWT (JSON Web Token) based authentication to secure all endpoints except the login endpoint.

=== Authentication Flow

1. **Login**: POST to `/auth/login` with admin credentials
2. **Token Generation**: Server returns JWT token with 30-minute expiry  
3. **Protected Access**: Include token in `Authorization: Bearer <token>` header
4. **Token Validation**: Server validates token on each request

=== Login Endpoint

.rest/src/main/java/payroll/security/AuthController.java
[%collapsible%open]
====
[source,java,tabsize=2]
----
...
public class AuthController {
    ...
    public ResponseEntity<Map<String, String>> login(@RequestBody LoginRequest loginRequest) {
        ...
        if (validUserId.equals(loginRequest.getUserId()) && 
            ...
            response.put("expiresIn", "1800"); // 30 minutes
...
.rest/src/main/java/payroll/security/JwtService.java
[%collapsible%open]
====
[source,java,tabsize=2]
----
...
public class JwtService {
    ...
    private static final long JWT_EXPIRATION = 30 * 60 * 1000; // 30 minutes
...
Spring Security is configured to protect all endpoints except login:

.rest/src/main/java/payroll/security/SecurityConfig.java
[%collapsible%open]
====
[source,java,tabsize=2]
----
...
[#_role_validation_and_error_handling]
== Enhanced Error Handling

We've implemented comprehensive error handling with JSON responses for better API consistency:

=== Employee Not Found Advice

.rest/src/main/java/payroll/EmployeeNotFoundAdvice.java
[%collapsible%open]
====
[source,java,tabsize=2]
----
...
=== Validation Error Advice

.rest/src/main/java/payroll/IllegalArgumentAdvice.java
[%collapsible%open]
====
[source,java,tabsize=2]
----
...
=== Authentication Error Advice

.rest/src/main/java/payroll/security/AuthenticationAdvice.java
[%collapsible%open]
====
[source,java,tabsize=2]
----
...
We provide comprehensive API documentation using OpenAPI 3.0 specification:

.rest/openapi.yaml
[%collapsible%open]
====
The complete OpenAPI specification includes:

* **Authentication endpoints** - Login flow with JWT tokens
* **Employee CRUD operations** - Full employee management with validation
* **Security schemes** - Bearer token authentication
* **Error responses** - Comprehensive error documentation  
* **Role validation** - Enum-based role constraints
* **Request/response examples** - Complete examples for all endpoints

Key features documented:
* JWT authentication with 30-minute expiry
* Role validation with specific enum values
...
**3. Try invalid role (gets 400 error):**
----
curl -X POST http://localhost:8080/employees \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
...
**4. Access without token (gets 401 error):**
----
curl -X GET http://localhost:8080/employees
----
...
Throughout this tutorial, you have engaged in various tactics to build production-ready REST APIs with enterprise-grade security. As it turns out, REST is not just about pretty URIs and returning JSON instead of XML.

The following features make your services robust, secure, and maintainable:

=== 🔐 **Security Best Practices**
* JWT-based authentication with configurable expiry
* Stateless authentication using Bearer tokens
* Protected endpoints with comprehensive error handling
* Secure credential validation

=== ✅ **Data Validation & Integrity**
* Enum-based role validation with clear error messages
* Input sanitization and blank field validation  
* Employee name uniqueness constraints
...
* Separation of concerns with dedicated exception handlers
...
=== 🎯 **REST & HATEOAS Compliance**
* Hypermedia-driven API design
* Self-descriptive messages with embedded links
* Support for API evolution without breaking clients
* Protocol-based state transitions

=== 🏗️ **Architecture & Design Patterns**
* Separation of concerns with dedicated service layers
* Dependency injection for testability and maintainability
* Repository pattern for data access
* Assembler pattern for hypermedia representation

This implementation demonstrates that building secure, validated, and well-documented REST APIs with Spring Boot requires minimal configuration while providing maximum functionality.

== Project Structure

This concludes our tutorial on how to build secure RESTful services using Spring. The project demonstrates:

* **Security**: JWT authentication with Spring Security
* **Validation**: Role-based validation with custom enums  
* **Error Handling**: Comprehensive JSON error responses
* **Documentation**: OpenAPI 3.0 specification
* **HATEOAS**: Hypermedia-driven API design
* **Testing**: Complete authentication and validation flows

The enhanced REST service provides a solid foundation for building production-ready APIs with enterprise-grade security and validation.
